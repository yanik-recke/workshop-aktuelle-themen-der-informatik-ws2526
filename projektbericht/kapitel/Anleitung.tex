...
\texttt{docker-compose.yml} beschrieben und lassen sich wie folgt starten.
...
\begin{itemize}
    \item \textbf{Docker} (inkl. Docker Compose)
    \item Ein \textbf{OpenAI-kompatibler API-Zugang}: entweder ein echter OpenAI API Key
    oder ein lokal laufendes Modell, das sich an die OpenAI-API hält
...
werden können. Alternativ lassen sich die Skripte auch direkt ausführen (direkte Ausführung klappt meist besser):
...
Dieser Schritt muss nur einmalig durchgeführt werden.

\section{Starten der Anwendung}

Sind die Dokumente indiziert, muss folgender Befehl ausgeführt werden:
...
Die Weboberfläche unter Port 3000 ist der Einstiegspunkt für den Benutzer.
...
Alternativ zur Weboberfläche steht eine Kommandozeilenschnittstelle
zur Verfügung. Diese half besonders während der Entwicklung und bei schnellen Tests:
